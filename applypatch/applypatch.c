...
// return 0 if it's a symlink. Otherwise return -1
int CheckSymLink(const char* filename) {
    struct stat st;
    // Be carefull. The default return value should be -1
    int ret = -1;

    // Skip partition files
...
int LoadFileContents(const char* filename, FileContents* file,
    ...
    // Output the file hash for debugging
    int i;
    printf("file:%s ", filename);
    for (i = 0; i < SHA_DIGEST_SIZE; i++){
        printf("%x", file->sha1[i]);
    }
