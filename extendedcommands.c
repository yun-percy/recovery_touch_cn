...
static const char *SDCARD_UPDATE_FILE = "/sdcard2/update.zip";
...
void write_recovery_version() {
    if ( is_data_media() ) {
        write_string_to_file("/sdcard2/0/clockworkmod/.recovery_version",EXPAND(RECOVERY_VERSION) "\n" EXPAND(TARGET_DEVICE));
    }
    write_string_to_file("/sdcard2/clockworkmod/.recovery_version",EXPAND(RECOVERY_VERSION) "\n" EXPAND(TARGET_DEVICE));
}
...
void show_install_update_menu()
{
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            {
                if (confirm_selection("Confirm install?", "Yes - Install /sdcard2/update.zip"))
                    install_zip(SDCARD_UPDATE_FILE);
                break;
            }
            case ITEM_CHOOSE_ZIP:
                show_choose_zip_menu("/sdcard2/");
...
{
    struct stat info;
    if (0 == stat("/sdcard2/clockworkmod/.no_confirm", &info))
        return 1;

    char* confirm_headers[]  = {  title, "  这将不能恢复.", "", NULL };
    int one_confirm = 0 == stat("/sdcard2/clockworkmod/.one_confirm", &info);
...
void show_partition_menu()
{
    ...
    for (;;)
    {
        ...
        if (!is_data_media()) {
          ...
          options[mountable_volumes + formatable_volumes] = "格式化 /data 和 /data/media (/sdcard2)";
          options[mountable_volumes + formatable_volumes + 1] = NULL;
        }
        ...
        if (chosen_item == (mountable_volumes+formatable_volumes)) {
            if (!is_data_media()) {
                ...
                if (!confirm_selection("格式化 /data 和 /data/media (/sdcard2)", confirm))
                    continue;
...
static void run_dedupe_gc(const char* other_sd) {
    ...
    nandroid_dedupe_gc("/sdcard2/clockworkmod/blobs");
    if (other_sd) {
            ...
            case 0:
                {
                    ...
                    if (tmp == NULL)
                    {
                        ...
                        sprintf(backup_path, "/sdcard2/clockworkmod/backup/%d", tp.tv_sec);
                    }
                    else
                    {
                        strftime(backup_path, sizeof(backup_path), "/sdcard2/clockworkmod/backup/%F.%H.%M.%S", tmp);
                    }
                ...
                break;
            case 1:
                show_nandroid_restore_menu("/sdcard2");
                write_recovery_version();
                break;
            case 2:
                show_nandroid_delete_menu("/sdcard2");
                write_recovery_version();
                break;
            case 3:
                show_nandroid_advanced_restore_menu("/sdcard2");
                write_recovery_version();
                break;
            case 4:
...
void process_volumes() {
    ...
    sprintf(backup_path, "/sdcard2/clockworkmod/backup/%s", backup_name);
...
void handle_failure(int ret)
{
    if (ret == 0)
        return;
    if (0 != ensure_path_mounted("/sdcard2"))
        return;
    mkdir("/sdcard2/clockworkmod", S_IRWXU | S_IRWXG | S_IRWXO);
    __system("cp /tmp/recovery.log /sdcard2/clockworkmod/recovery.log");
    ui_print("/tmp/recovery.log was copied to /sdcard2/clockworkmod/recovery.log. Please open ROM Manager to report the issue.\n");
}
