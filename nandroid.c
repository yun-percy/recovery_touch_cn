...
void nandroid_generate_timestamp_path(const char* backup_path)
{
    ...
    {
        strftime(backup_path, PATH_MAX, "/sdcard2/clockworkmod/backup/%F.%H.%M.%S", tmp);
    }
}
...
int nandroid_backup_partition_extended(const char* backup_path, const char* mount_point, int umount_when_finished) {
    ...
    int callback = stat("/sdcard2/clockworkmod/.hidenandroidprogress", &file_info) != 0;
...
int nandroid_backup(const char* backup_path)
{
    ...
    if (is_data_media() || 0 != stat("/sdcard2/.android_secure", &s)) {
        ui_print("没有发现 /sdcard2/.android_secure . 将自动跳过对外置存储设备中程序的备份.\n");
    }
    else {
        if (0 != (ret = nandroid_backup_partition_extended(backup_path, "/sdcard2/.android_secure", 0)))
            return ret;
    }
    ...
    sprintf(tmp, "chmod -R 777 %s ; chmod -R u+r,u+w,g+r,g+w,o+r,o+w /sdcard2/clockworkmod ; chmod u+x,g+x,o+x /sdcard2/clockworkmod/backup ; chmod u+x,g+x,o+x /sdcard2/clockworkmod/blobs", backup_path);
    __system(tmp);
    sync();
...
int nandroid_restore(const char* backup_path, int restore_boot, int restore_system, int restore_data, int restore_cache, int restore_sdext, int restore_wimax)
{
    ...
    if (restore_data && 0 != (ret = nandroid_restore_partition_extended(backup_path, "/sdcard2/.android_secure", 0)))
        return ret;
